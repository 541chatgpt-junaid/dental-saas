...
export default function StaffPage() {
  ...
  const handleAdd = async () => {
    ...
    if (result.success) {
      setInviteMsg("Invite email sent to " + form.email);
    } else {
      setInviteMsg("Note: " + (result.error || "Could not send invite"));
    }
    ...
    <div className="min-h-screen flex bg-teal-50">
      ...
      <div className="flex-1 p-4 md:p-8 mt-14 md:mt-0">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-xl md:text-2xl font-semibold text-teal-800">Staff Management</h2>
        ...
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6">
          <p className="text-sm text-blue-700 font-medium">Staff Invite System</p>
          <p className="text-xs text-blue-500 mt-1">Jab aap staff add karenge — unhe automatically email invite jayega. Woh link click karke password set kar sakte hain!</p>
        </div>
          ...
          <div className="bg-white rounded-xl p-6 border border-teal-100 mb-6">
            ...
            <div className="flex gap-3">
              <button onClick={handleAdd} disabled={loading} className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-2 rounded-lg text-sm font-medium disabled:opacity-60">
                {loading ? "Sending Invite..." : "Add Staff & Send Invite"}
              </button>
                  ...
                  <tr key={s.id} className="border-t border-teal-50 hover:bg-teal-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {s.permissions ? s.permissions.split(",").slice(0, 3).map(p => (
                          <span key={p} className="bg-teal-100 text-teal-700 text-xs px-2 py-0.5 rounded-full">{p}</span>
                        )) : <span className="text-gray-400 text-xs">No permissions</span>}
                        {s.permissions && s.permissions.split(",").length > 3 && (
                          <span className="bg-teal-100 text-teal-700 text-xs px-2 py-0.5 rounded-full">+{s.permissions.split(",").length - 3} more</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        <button onClick={() => openEdit(s)} className="text-teal-600 hover:text-teal-800 text-xs font-medium">Edit Permissions</button>
