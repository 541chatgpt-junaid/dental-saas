    ...
    const draw = () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        for (let x = 0; x <= canvas.width; x += 5) {
          const y =
            canvas.height * 0.5 +
            Math.sin(x * 0.003 + t + i * 0.8) * (60 + i * 20) +
            Math.sin(x * 0.007 + t * 1.3 + i) * 30;
          ctx.lineTo(x, y);
        }
    ...
    <>
      ...
      <nav
        style={{
          ...
          borderBottom: navScrolled ? "1px solid rgba(10,22,40,0.08)" : "none",
          boxShadow: navScrolled ? "0 2px 20px rgba(10,22,40,0.06)" : "none",
          transition: "all 0.3s",
        }}
      >
        <div style={{ width: "100%", maxWidth: "1200px", margin: "0 auto", padding: "16px 32px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "16px" }} className="hidden md:flex">
            ...
            <Link href="/login">
              <button
                style={{ background: "#0A1628", color: "#fff", borderRadius: "999px", padding: "10px 22px", fontSize: "14px", fontWeight: 600, border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", transition: "all 0.2s" }}
        ...
        {mobileMenu && (
          <div className="md:hidden" style={{ padding: "0 24px 20px", borderTop: "1px solid rgba(10,22,40,0.06)", background: "#fff", display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            <Link href="/login">
              <button style={{ width: "100%", padding: "12px", borderRadius: "999px", fontSize: "14px", fontWeight: 600, color: "#fff", background: "#0A1628", border: "none", cursor: "pointer" }}>
                Get Started Free
              </button>
            </Link>
          </div>
      ...
      <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", paddingTop: "80px", background: "linear-gradient(160deg, #dde8ff 0%, #e8eeff 40%, #d8e4ff 100%)", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: "860px", margin: "0 auto", padding: "0 24px", textAlign: "center" }}>
          ...
          <Link href="/login">
            <button
              style={{ background: "#0A1628", color: "#fff", borderRadius: "999px", padding: "18px 40px", fontSize: "16px", fontWeight: 600, border: "none", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: "10px", transition: "all 0.25s", boxShadow: "0 4px 24px rgba(10,22,40,0.15)" }}
