...
interface PrescriptionPreset { id: string; medicine: string; dosage: string; frequency: string; duration: string; notes: string; }
...
export default function Settings() {
  ...
  const [pForm, setPForm] = useState({ medicine: "", dosage: "", frequency: "", duration: "", notes: "" });
  const [pEditId, setPEditId] = useState<string | null>(null);
  const [pEditForm, setPEditForm] = useState({ medicine: "", dosage: "", frequency: "", duration: "", notes: "" });
  ...
  const addPrescription = async () => {
    ...
    setPLoading(true);
    await createClient().from("prescription_presets").insert([{ clinic_id: clinicId, medicine: pForm.medicine.trim(), dosage: pForm.dosage, frequency: pForm.frequency, duration: pForm.duration, notes: pForm.notes }]);
    setPForm({ medicine: "", dosage: "", frequency: "", duration: "", notes: "" });
    await fetchPrescriptions();
    setPLoading(false);
  };

  const savePrescriptionEdit = async (id: string) => {
    await createClient().from("prescription_presets").update({ medicine: pEditForm.medicine.trim(), dosage: pEditForm.dosage, frequency: pEditForm.frequency, duration: pEditForm.duration, notes: pEditForm.notes }).eq("id", id);
    setPEditId(null);
    fetchPrescriptions();
  };
    ...
    <div className="min-h-screen flex bg-gray-100">
      ...
      <div className="flex-1 p-4 md:p-8 mt-14 md:mt-0">
        ...
        {tab === "prescriptions" && (
          <div className="max-w-2xl space-y-5">
            <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 text-xs text-blue-800">
              These appear as quick-add buttons when adding visit notes for a patient. Click to auto-fill the prescription field.
            </div>

            {/* Add form */}
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
              <h3 className="text-sm font-semibold text-gray-800 mb-4">Add Prescription Preset</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input placeholder="e.g. 1 tablet, 500mg" value={pForm.dosage} onChange={e => setPForm({ ...pForm, dosage: e.target.value })}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">Frequency</label>
                  <input placeholder="e.g. 2x daily, 3x daily" value={pForm.frequency} onChange={e => setPForm({ ...pForm, frequency: e.target.value })}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">Duration</label>
                  <input placeholder="e.g. 5 days, 1 week" value={pForm.duration} onChange={e => setPForm({ ...pForm, duration: e.target.value })}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">Instructions</label>
                  <input placeholder="e.g. After meals, with water" value={pForm.notes} onChange={e => setPForm({ ...pForm, notes: e.target.value })}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
                </div>
              </div>
              <button onClick={addPrescription} disabled={pLoading || !pForm.medicine.trim()}
                className="mt-4 bg-teal-600 hover:bg-teal-700 text-white px-5 py-2.5 rounded-xl text-sm font-medium disabled:opacity-50">
                {pLoading ? "Adding..." : "+ Add Prescription Preset"}
              </button>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="px-5 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-gray-700">Your Prescription Presets</h3>
                <span className="text-xs text-gray-400">{prescriptions.length} presets</span>
              </div>
              {prescriptions.length === 0 ? (
                <p className="text-sm text-gray-400 text-center py-10">No prescription presets yet. Add your first one above.</p>
              ) : (
                <div className="divide-y divide-gray-50">
                      ...
                      {pEditId === rx.id ? (
                        ...
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex gap-3 shrink-0">
                            <button onClick={() => { setPEditId(rx.id); setPEditForm({ medicine: rx.medicine, dosage: rx.dosage, frequency: rx.frequency, duration: rx.duration, notes: rx.notes }); }}
                              className="text-xs text-teal-600 font-medium hover:underline">Edit</button>
